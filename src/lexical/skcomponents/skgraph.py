from src.lexical.skcomponents.sknode import Node
from src.lexical.skcomponents.sknodeset import NodeSet
...
class Graph(NodeSet):
    ...
    def _parse_line_to_node(self, line, line_number):
        ...
        try:
            ...
            favorite = parts[1] == 'T'
            synsets = [part.split('/') for part in parts[2:5]]
            semsets = [part.split('/') for part in parts[5:8]]
        ...
        with open(file_path, 'w', encoding='utf8') as file:
            for node in self:
                try:
                    ...
                    for attr in self.EDGE_TYPES:
                        line += '|' + '/'.join([string_ids[n] for n in getattr(node, attr)])
        ...
        for node in self:
            for attr in self.EDGE_TYPES:
                ...
                if len(new_attrs) != len(getattr(node, attr)):
                    print(f"Removed spurious edges from {node.name}")
    ...
    def _check_self_connections(self):
        ...
        for node in self:
            for attr in self.EDGE_TYPES:
                ...
                if len(new_attrs) != len(getattr(node, attr)):
                    print(f"Removed self-connection from {node.name}")
                setattr(node, attr, new_attrs)
        print('OK')
        ...
        for node in self:
            for attr in self.EDGE_TYPES:
                ...
                if len(attrs) != len(set_attrs):
                    print(f"Removed redundancy in {node.name}")
    ...
    def bind(self, target_node, append_node, target_edge_type):
        if target_node != append_node and append_node not in getattr(target_node, target_edge_type, []):
    ...
    def _update_reciprocal_edges(self, node_a, node_b, edge_type_a, operation):
        edge_type_a = sk.parse_field(edge_type_a, long=True)[0]
